...
use sui_types::traffic_control::{ClientId, PolicyConfig, RemoteFirewallConfig, Weight};
use tokio::sync::mpsc;
use tokio::sync::mpsc::error::TrySendError;
use tracing::{debug, error, info, warn};

type Blocklist = Arc<DashMap<ClientId, SystemTime>>;
...
impl TrafficSim {
    ...
    async fn run_single_client(
        ...
        while start.elapsed() < duration {
            let client = Some(ClientId::IpAddr(IpAddr::V4(Ipv4Addr::new(
                127, 0, 0, task_num,
            ))));
