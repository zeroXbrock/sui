...
use sui_types::traffic_control::{ClientId, FreqThresholdConfig, PolicyConfig, PolicyType, Weight};
use tracing::info;
...
#[derive(Hash, Eq, PartialEq, Debug)]
struct SketchKey(ClientId, ClientType);
...
pub struct TrafficTally {
    ...
    pub proxied_client: Option<ClientId>,
...
impl TrafficTally {
    pub fn new(
        ...
        proxied_client: Option<ClientId>,
        error_weight: Weight,
    ) -> Self {
        Self {
            client,
...
pub struct PolicyResponse {
    pub block_client: Option<ClientId>,
    pub block_proxied_client: Option<ClientId>,
}
...
impl FreqThresholdPolicy {
    ...
    fn handle_tally(&mut self, tally: TrafficTally) -> PolicyResponse {
        let block_client = if let Some(source) = tally.client {
            let key = SketchKey(source.clone(), ClientType::Client);
        ...
        let block_proxied_client = if let Some(source) = tally.proxied_client {
            let key = SketchKey(source.clone(), ClientType::ProxiedClient);
            self.sketch.increment_count(&key);
...
pub struct TestNConnIPPolicy {
    config: PolicyConfig,
    frequencies: Arc<RwLock<HashMap<ClientId, u64>>>,
    threshold: u64,
}
...
impl TestNConnIPPolicy {
    ...
    fn handle_tally(&mut self, tally: TrafficTally) -> PolicyResponse {
        let client = if let Some(client) = tally.client {
            client
        } else {
        ...
        let count = frequencies.entry(client.clone()).or_insert(0);
        *count += 1;
        PolicyResponse {
...
async fn run_clear_frequencies(frequencies: Arc<RwLock<HashMap<ClientId, u64>>>, window_secs: u64) {
    loop {
...
mod tests {
    ...
    async fn test_freq_threshold_policy() {
        ...
        let alice = TrafficTally {
            client: Some(ClientId::IpAddr(IpAddr::V4(Ipv4Addr::new(8, 7, 6, 5)))),
            proxied_client: Some(ClientId::IpAddr(IpAddr::V4(Ipv4Addr::new(1, 2, 3, 4)))),
            error_weight: Weight::zero(),
            timestamp: SystemTime::now(),
        };
        let bob = TrafficTally {
            client: Some(ClientId::IpAddr(IpAddr::V4(Ipv4Addr::new(8, 7, 6, 5)))),
            proxied_client: Some(ClientId::IpAddr(IpAddr::V4(Ipv4Addr::new(4, 3, 2, 1)))),
            error_weight: Weight::zero(),
            timestamp: SystemTime::now(),
        };
        let charlie = TrafficTally {
            client: Some(ClientId::IpAddr(IpAddr::V4(Ipv4Addr::new(8, 7, 6, 5)))),
            proxied_client: Some(ClientId::IpAddr(IpAddr::V4(Ipv4Addr::new(5, 6, 7, 8)))),
            error_weight: Weight::zero(),
            timestamp: SystemTime::now(),
        };
        ...
        let response = policy.handle_tally(bob.clone());
        assert_eq!(response.block_client, None);
        assert_eq!(response.block_proxied_client, bob.proxied_client);
        ...
        let response = policy.handle_tally(alice.clone());
        assert_eq!(response.block_client, None);
        assert_eq!(response.block_proxied_client, alice.proxied_client);
        ...
        let response = policy.handle_tally(charlie.clone());
        assert_eq!(response.block_proxied_client, None);
        assert_eq!(response.block_client, charlie.client);
