...
use sui_types::traffic_control::{ClientId, RemoteFirewallConfig};
...
async fn process_raw_request<L: Logger>(
    ...
    let client = match service.client_id_source {
        Some(ClientIdSource::SocketAddr) => Some(ClientId::IpAddr(client_addr.ip())),
        Some(ClientIdSource::XForwardedFor) => {
            // TODO - implement this later. Will need to read header at axum layer.
            error!("X-Forwarded-For client ID source not yet supported on json rpc servers. Skipping traffic controller request handling.");
            None
        }
        None => None,
    };
